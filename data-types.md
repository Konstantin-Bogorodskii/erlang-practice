...
2 + 5. -> 7
49 \* 100. -> 4900
5 div 2. -> 2 --> сколько двоек в 5
5 rem 2. -> 1 --> остаток от деления
2#101010. -> 42 --> целое число в системе счисления основание#число

Числа в Erlang безразмерные - это значит что туда можно положить число любой длины

====== Методы =====

1. math:pow(10, 40) -> 0e40
2. round(math:pow(10, 40)). - 10000000000000000303786028427003666890752
3. abs(1) -> 1, abs(-1) -> 1.
...
!!! На самом деле, = — это оператор сопоставления по образцу, который ведет себя как оператор присвоения. В большинстве языков символ = означает оператор присваивания. Тем не менее в Эрланге символ = означает операцию сопоставления по образцу. Выражение Л = П означает на самом деле следующее: вычисляется правая сторона (П) и потом она сопоставляется образцу в левой стороне выражения (Л). X = 6. и X = 2 + 4. не вызовет ошибки. X = 6. X = 1 + 4. Выдаст ошибку !!!
One + On. -> 2
...
!!! Ссылочная прозрачность - это термин свойственнен многим функциональным языкам программированию. В данном примере Erlang мы создаём переменную List в котору по ССЫЛКЕ "=" кладём значения списка [1,2,3,4,5]. При этом Erlang гарантирует что данная ссылка как и значение переменной никогда не будут изменены !!!
List = [1,2,3,4,5].

Все переменные в Erlang не изменяемы (immutable), поэтому нельзя им присвоить новое значение: X = 87. - здесь будет ошибка.
На самом деле в Erlang не выполняется присваивание, вместо этого выполняется одновременно два действия: указанная переменная связывается (bound) со значением и выполняется сопоставление с бразцом (pattern matched).
...
Атомы можно использовать для обазначения пола пользователья male, female.
...
1. hd(NewList). -> [1] или tl(NewList). -> [2,3,4]
2. [Head | Tail] = NewList.

Оператор | - называется конкатенации и используется для склейки двух спсков [1 | []]. -> [1 | [2,3 |[5,6]]]. Запятая при этом между списками не нужна.
При это все начинается с крайнего правого списка [5,6] и к этому спика добавляется элемент [2,3] и т.д.

!!! Время выполнения операций со списками !!!

1. Доступ к N-му элементу списка осуществляется за время O(n). Это является слабость функциональных языков в отличие от императивных(js). Где доступ к элементу по индексу или добавление в конец массива выполняется за константное время.
2. Добавление нового элемента к списку [1 | List] или извлечение [Head | Tail] = List. выполлянются за константное время
3. List ++ List1 - выполняется за O(n), где n - кол-во элементов в первом спике(List). При это кол-во элементов второго списка не учитывается. Второй список беретс из памяти.
...
1. length(List) - вернёт длину списка.
2. lists:nth(2, List) - получить 2-й элемент списка List.
3. lists:reverse(List) - перевернуть списко.
4. lists:map(F, L). Она возвращает список, созданный применением функции F к каждому элементу из списка L:
5. lists:filter(P, L), она возвращает новый список из таких элементов E списка L, для которых функция P(E) равна true.

6. lists:member(X, L) возвращает true если X находится в списке L, в противном случае она возвращает false. Давайте протестируем нашу функцию:

- Fruit = [apple,pear,orange].
- MakeTest = fun(L) -> (fun(X) -> lists:member(X, L) end) end.
- IsFruit = MakeTest(Fruit).

**_ Обработчики списков _**

1. L = [1,2,3,4,5].
2. [2*X || X <- L ]. -> [2,4,6,8,10]
...
1. Сочетание знаков (~n) это управляющий символ перевода строки.

29> io:format("hello world!").
hello world!ok
29> io:format("hello world!~n").
hello world!
ok

2. io:format("Hello, World!~n", [])

Оператор вывода io:format на самом деле является функцией format из модуля io.
!!! . Внутри текста в двойных кавычках можно вставлять комбинацию знаков ~w и тогда на место этих знаков при выводе будет вставлен элемент из списка: io:format("результат: ~w~n ", [math:pow(3,2)]). → результат: 9.0 !!!
Можно делать несколько вставок:

- io:format("Его зовут ~w, ему ~w лет~n", [ivan, 25]). → Его зовут ivan, ему 25 лет
- io:format("Его зовут ~p, ему ~w лет~n", ["Иван", 25]). → Его зовут "Иван", ему 25 лет

===== Методы =====
string:to_upper(String) - в верхний регистер
...
=:= - это проверка тип + значение, == - проверка только на значение
=/= - это проверка тип + значение, /= - проверка только на значение
